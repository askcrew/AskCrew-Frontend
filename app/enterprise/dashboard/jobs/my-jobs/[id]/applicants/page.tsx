import {
  JobApplicantsDataTable,
  type JobApplicant,
} from "@/components/enterprise/jobs/job-applicants-data-table";
import applicantsData from "@/components/enterprise/jobs/job-applicants-data-table/data.json";
import { Button } from "@/components/ui/button";
import { IconArrowLeft } from "@tabler/icons-react";
import Link from "next/link";

const applicants = applicantsData as JobApplicant[];

const JobApplicantsPage = () => {
  // In a real app, fetch job details using params.id
  const jobTitle = "Lead Actor - Drama Series";
    ...
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="relative overflow-hidden rounded-xl bg-linear-to-br from-orange-500/10 via-purple-500/10 to-pink-500/10 p-8 border border-orange-200/20 dark:border-orange-500/20">
        ...
        <div className="relative flex flex-col gap-3">
          {/* Back Button */}
          <Link href="/enterprise/dashboard/jobs/my-jobs">
            <Button
              variant="ghost"
              size="sm"
          ...
          <p className="text-muted-foreground text-lg max-w-2xl">
            Review and manage applications for your job posting. Accept or
            reject applicants based on their profiles, experience, and
            qualifications.
          </p>
      ...
      {/* Data Table */}
      <JobApplicantsDataTable data={applicants} />
    </div>
  );
};
