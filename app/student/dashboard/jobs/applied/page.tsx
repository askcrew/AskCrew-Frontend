"use client"
import {
  AppliedJobsDataTable,
  type AppliedJob,
} from "@/components/enterprise/jobs/applied-jobs-data-table";
import appliedJobsData from "@/components/enterprise/jobs/applied-jobs-data-table/data.json";
import axiosInstance from "@/lib/axiosInstance";
import { useQuery } from "@tanstack/react-query";

const appliedJobs = appliedJobsData as AppliedJob[];

const AppliedToJobsPage = () => {
  const { data, isLoading, error } = useQuery({
     queryKey: ["my-applications"],
     queryFn: async () => {
       const res = await axiosInstance.get("/community/applications");
       return res.data;
     },
   });
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="relative overflow-hidden rounded-xl bg-linear-to-br from-orange-500/10 via-purple-500/10 to-pink-500/10 p-8 border border-orange-200/20 dark:border-orange-500/20">
        ...
        <div className="relative flex flex-col gap-3">
          ...
          <p className="text-muted-foreground text-lg max-w-2xl">
            Track your job applications and share your experience. Review
            concluded positions to help others make informed decisions.
          </p>
          ...
          <div className="flex gap-4 mt-4">
            <div className="flex items-center gap-2 px-4 py-2 rounded-lg bg-orange-500/10 border border-orange-500/20">
              ...
              <span className="text-sm font-medium text-orange-700 dark:text-orange-400">
                {data?.results?.filter((j) => j.status === "pending").length}{" "}
                Pending
              </span>
            </div>
            <div className="flex items-center gap-2 px-4 py-2 rounded-lg bg-purple-500/10 border border-purple-500/20">
              ...
              <span className="text-sm font-medium text-purple-700 dark:text-purple-400">
                {data?.results?.filter((j) => j.status === "accepted").length}{" "}
                Accepted
              </span>
            </div>
            <div className="flex items-center gap-2 px-4 py-2 rounded-lg bg-pink-500/10 border border-pink-500/20">
              ...
              <span className="text-sm font-medium text-pink-700 dark:text-pink-400">
                {data?.results?.filter((j) => j.status === "rejected").length}{" "}
                Rejected
              </span>
            </div>
          </div>
      ...
      {/* Data Table */}
      <AppliedJobsDataTable data={data?.results} />
    </div>
  );
};
